...
The static site build (`pnpm build`) then picks up `public/data/*` automatically. You can still override paths using CLI flags on each script.
...
data/laws/
  ...
  cache/
    ...
    gii_metadata.json              # Cached GII metadata (jurabk/kurzue/ausfertigung_datum)
    extraction_progress.json       # Resume checkpoint for LLM extraction
